...
const AddBlog = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const photoFile = form.file.files[0];
    const description = form.description.value;
    // email এর ভ্যালু এখন form থেকে আসবে কারণ ইনপুটে readOnly আছে
    const email = form.email.value;

    // যদি কোন ফাইল না থাকে, তাহলে এ্যালার্ট দেখাবে
    if (!photoFile) {
      Swal.fire({
        position: "top-center",
        icon: "error",
    ...
    // ইমেজ আপলোড করার জন্য FormData তৈরি করা
    const formData = new FormData();
    formData.append("image", photoFile);

    try {
      // imgBB API তে ইমেজ আপলোড করা
      const response = await axios.post(uploadUrl, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      ...
      if (response.data.success) {
        const imageUrl = response.data.data.url;
        // email সহ ব্লগ ডাটা তৈরি
        const blogData = { title, image: imageUrl, description, email };

        await axios.post(`${import.meta.env.VITE_SOME_KEY}/blog`, blogData);
